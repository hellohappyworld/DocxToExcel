...
def writeExcle():
    ...
    active.cell(maxRow + 1, 5, kv["订单编号"])
    active.cell(maxRow + 1, 6, kv["供货单号"])
    active.cell(maxRow + 1, 7, kv["项目单位/收货方"])
    active.cell(maxRow + 1, 8, kv["项目名称"])
    active.cell(maxRow + 1, 9, kv["货物名称"])
    active.cell(maxRow + 1, 10, kv["数量"])
    active.cell(maxRow + 1, 11, kv["分项价（含税）"])
    allMoney = float(kv["分项价（含税）"])
    num = float(kv["数量"])
    danjia = round(allMoney / num, 2)
    active.cell(maxRow + 1, 12, danjia)
    active.cell(maxRow + 1, 13, kv["交货时间"])
    active.cell(maxRow + 1, 14, kv["支付比例"])
